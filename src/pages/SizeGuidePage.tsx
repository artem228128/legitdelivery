...
const sizeData = {
  ...
  "Yeezy": {
    ...
    "Woman": [
      ...
      { "EU": "39.5", "Adidas EU": "41", "UK": "8", "US": "7.5", "cm": 25.5 }
    ]
  }
};
  ...
  const calculateSize = () => {
    ...
    if (!footLength) {
      ...
      if (length < 21 || length > 32) {
        ...
        // Проверяем соответствие размера полу только если пол выбран
        if (gender === 'Man' && length < 25) {
          newErrors.footLength = 'Для чоловіків мінімальна довжина стопи 25 см';
        } else if (gender === 'Woman' && length > 28) {
          newErrors.footLength = 'Для жінок максимальна довжина стопи 28 см';
        }
      }
    }
    ...
    // Проверяем, имеет ли бренд структуру с полом
    let brandSizes: any[];
    if (brandData && typeof brandData === 'object' && 'Man' in brandData && 'Woman' in brandData) {
      // Бренд имеет структуру с полом
      brandSizes = (brandData as any)[gender];
    } else {
      // Бренд имеет простую структуру массива
